...
@NLconstraint(mp, GdPsinter[i = 1:psupplynum], sum(GdflowinPs[i][j] for j in 1:length(GdflowinPs[i])) == (dt.au + dt.bu*Pload[i] + dt.cu*Pload[i]^2)/(dt.H*dt.gasdensity)*3600*1000)
...
# PdGlflowout2, Pr1_2, Pr2_2 = sf.zeropad(PdGlflowout2), sf.zeropad(Pr1_2), sf.zeropad(Pr2_2)
 ...
 @NLconstraint(mp, Pdwglink[i = 1:pdemandnum], 2.7e-7*sum(dt.wdensity*dt.g*PdWlflowout1[i][j]*(H2_1[i][j] - H1_1[i][j]) for j in 1:length(H2_1[i])) +
            2.7e-7*sum(10.654*(PdWlflowout1[i][j]/dt.beta)^1.852*(L_1[i][j]/Waterdict["edgediameter"]) for j in 1:length(H2_1[i])) +
            2.7e-7*sum(dt.wdensity*dt.g*PdWlflowout2[i][j]*(H2_2[i][j] - H1_2[i][j]) for j in 1:length(H2_2[i])) +
            2.7e-7*sum(10.654*(PdWlflowout2[i][j]/dt.beta)^1.852*(L_2[i][j]/Waterdict["edgediameter"]) for j in 1:length(H2_2[i])) +
